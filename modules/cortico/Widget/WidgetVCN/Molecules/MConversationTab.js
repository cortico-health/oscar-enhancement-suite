...
const MConversationTab = ({conversation, selected, ...props}) => {
  ...
  const isSelected = conversationStore.conversations.selected?.id == conversation.id;
  ...
  return (
    <div
      ...
      className={ classNames(
        "tw-flex tw-flex-column tw-items-center tw-px-2 tw-my-3 tw-cursor-pointer",
        isSelected ? "tw-h-24 tw-rounded-lg tw-bg-primary-500" : "",
        unreadMessages ? "tw-font-bold" : ""
      ) }>
      <div
        ...
        <div className="tw-max-w-full tw-ml-4 tw-cursor-pointer">
          <p
            className={ `tw-text-contact2 tw-w-72 tw-cursor-pointer tw-whitespace-nowrap tw-text-ellipsis tw-overflow-hidden ${isSelected
                  ? "tw-text-white tw-font-bold"
                  : "tw-text-secondary-500"
              }` }>
            { multipleObjectDataFormatting(
              getOtherMembersName(conversation?.members)
            ) }
            { conversation?.patient && (
              <span className="tw-cursor-pointer">
                { " " }
                ({ conversation.patient.first_name }{ " " }
                { conversation.patient.last_name })
              </span>
            ) }
          </p>
          <p
            className={ `tw-text-contact3 tw-relative tw-cursor-pointer tw-mt-2 tw-w-72 tw-whitespace-nowrap tw-text-ellipsis tw-overflow-hidden ${isSelected
                  ? "tw-text-white tw-font-bold"
                  : "tw-text-secondary-300"
              }` }>
            { lastMessage
              ? `${lastMessage.from_user}: ${lastMessage.body}`
              : "" }
          </p>
        </div>
        { unreadMessages > 0 && (
