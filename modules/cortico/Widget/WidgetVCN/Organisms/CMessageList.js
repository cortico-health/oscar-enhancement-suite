...
const CMessageList = ({isEncounterPage}) => {
  ...
  const handlers = {
    ...
    removeFile: (id) => {
      setUploadedFiles((prevUploadedFiles) =>
        prevUploadedFiles.filter((uploadedFile) => uploadedFile.id !== id)
      );
    },
    onSend: () => {
  ...
  useEffect(() => {
    ...
    if (conversation) {
      setLoading(true);
      getChatMessageData(conversation.id)
        .then((response) => {
          return response.json();
        })
        .then((data) => {
          loadExtensionStorageValue("jwt_access_token").then((accessToken) => {
            if (accessToken)
              setSocketUrl(getWsChatUrl(conversation.id, accessToken));
          });
  ...
  useEffect(() => {
    if (conversationStore.conversations.selected?.stats)
      setFileStats(conversationStore.conversations.selected.stats);
                        ...
                        return (
                          <MMessageCard
                            ...
                            readHistory={readHistory.filter(
                              (m) => m.chat_message_id === message.id
                            )}
                  ...
                  <div className="tw-sticky tw-border tw-bg-secondary-10 tw-w-full">
                    <div className="tw-mx-12">
                        ...
                        <div className="tw-flex tw-flex-wrap tw-w-full tw-max-h-32 tw-overflow-y-auto tw-mt-3">
                          {uploadedFiles.map((file) => {
                            return (
                              <AFileInputShow
                                fileInput={file}
                                exit={() => handlers.removeFile(file.id)}
                              />
                            );
                          })}
                        </div>
                      ...
                      <p className="tw-text-h3 tw-text-right tw-text-secondary-500 tw-pb-4">
                        ...
                        <span
                          className="tw-font-medium tw-text-primary-500 tw-cursor-pointer"
                          onClick={goToSelectPatient}>
