...
const MSelectItem= ({className, patient, selected, ...props}) => {
  ...
  return (
    <tr {...props} className={` ${selected ? 'bg-primary-500': ''} cursor-pointer`}>
     ...
     <td><p className={`${selected ? 'text-white' : 'text-secondary-200'} text-select2 font-medium`}>{ patient.birthDate.toLocaleDateString() }</p></td>
     <td><p className={`${selected ? 'text-white' : 'text-secondary-500'} text-select2 font-medium`}>{ patient.healthCardNumber }</p></td>
