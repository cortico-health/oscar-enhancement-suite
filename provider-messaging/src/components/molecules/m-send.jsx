...
const MSend = ({ handlers, className, ...props}) => {
  ...
  return (
    <AInput onKeyDown={handlers.onSend} type='textarea' name='send' {...props} className={`m-send ${className}`}>
      <label htmlFor="file">
        <ASvg className='absolute cursor-pointer pointer-events-auto left-2 top-1/2 -translate-y-1/2' src="attach"/>
      ...
      <div onClick={handlers.onSend} >
        <ASvg className='absolute cursor-pointer pointer-events-auto right-2 top-1/2 -translate-y-1/2' src="send"/>
      </div>
    </AInput>
  )
}
