...
const AButton = ({variant='button-primary-sm', isLoading = false, href, children, className, ...props}) => {
  ...
  return (
      href==undefined ?
        <button className={`a-button my-3 rounded ${variantClass[variant]} ${className? className: ''}`} {...props} >
          <LoadingButton isLoading={isLoading}/>
          {children}
        </button> :
        <a href={href} className={`a-button my-3 flex justify-center items-center rounded ${variantClass[variant]} ${className}`} {...props} >
          <LoadingButton isLoading={isLoading}/>
          {children}
        </a>
  )
