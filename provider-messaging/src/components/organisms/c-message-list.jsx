...
const CMessageList = () => {
  ...
  useEffect(() => {
    if (router.matches?.id) {
      ...
      getChatMessageData(router.matches?.id, authStore.accessToken)
        .then((response) => {
          setSocketUrl(
            `${
              import.meta.env.VITE_WEBSOCKET_URL || "ws://localhost:8426"
            }/chat/${router.matches?.id}/?token=${authStore.accessToken}`
          );
